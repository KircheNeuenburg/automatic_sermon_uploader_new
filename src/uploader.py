...
def convert_video_to_audio(file_path, video_extension, audio_extension):
    ...
    audio_path = file_path.replace("." + video_extension, "." + audio_extension)
    clip = mp.VideoFileClip(file_path).subclip(0)
    clip.audio.write_audiofile(audio_path)
    return audio_path


#convert_video_to_audio("test.avi", "avi", "mp3")
...
def get_sermon_metadata(file_path):
    ...
    regex_match = re.match(r"(?P<date>[0-9]{4}-[0,1][0-9]-[0-3][0-9])_(?P<title>[\W\w]+)_(?P<preacher>[\W\w]+)[.][\W\w]+", file_name)

    if regex_match:
        date_raw = regex_match.group("date").split("-")

        metadata["date"] = datetime.date(int(date_raw[0]), int(date_raw[1]), int(date_raw[2]))
...
#print(get_sermon_metadata("2017-02-19_Titel der Predigt - \
# Was kann Gott erreichen_Pfr. Armin Graf-Eckhard.mp4"))
...
def upload_video_to_vimeo(config, video_path, metadata):
    ...
    vimeo_handle.patch(
        video_uri, \
        data={'name': metadata["title"] + " // " + metadata["preacher"] + \
...
#print(upload_video_to_vimeo(load_config("../config.json"),"test.mp4",\
# get_sermon_metadata("2017-02-19_Titel der Predigt - \
# 95 Thesen_Pfr. Martin Luther.mp4")))
...
def create_wordpress_post(config, video_url, audio_url, metadata):
    ...
    category = wordpress_handle.call(taxonomies.GetTerm('category', \
        config["wordpress"]["category_id"]))
    ...
    if audio_url is not None:
        download_html = "<a style=\"text-decoration:none; background-color:#0076b3; \
            border-radius:3px; padding:5px; color:#ffffff; border-color:black; border:1px;\"\
            href=\"" + audio_url + "\" title=\"Download als MP3\" \
            target=\"_blank\">Download als MP3</a>"
        audio_html = "<div><h3>Audiopredigt:</h3><audio controls src=\"" + audio_url + \
            "\"></audio></div>"
    else:
    ...
    date_time = datetime.datetime(metadata["date"].year, \
        metadata["date"].month, metadata["date"].day, config["sermon_start_utc"])
...
#create_wordpress_post(load_config("../config.json"), "/video/204783966", \
# "http://heise.de/index.html", \
#    get_sermon_metadata("2017-02-19_Titel der Predigt - 95 Thesen_Pfr. Martin Luther.mp4"))
